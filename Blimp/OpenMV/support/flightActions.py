...
class FlightAction:
    ...
    def execute(self):
        ...
        logger.log.info("Controls Values: {}".format(
        ...
        for item in self.exitCriteria.list:
            if not self.isCriteriaTimeClock(item.variableName):
                if item.value == self.getProperty(item.variableName):
                    ...
                    logger.log.verbose("NO MATCH (" + item.variableName + ") = " + str(
                        item.value) + " != " + str(self.getProperty(item.variableName)))
    ...
    def execute_assisted_altitude(self, height):
        ...
        if height != None:
            self.controls.up = self.pid_up.get_pid(
                height-self.data.lidarDistance)
            logger.log.info(
                "Executing Assisted Altitude.  PID Up Value: {}".format(self.controls.up))
