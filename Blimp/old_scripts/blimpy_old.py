...
ps3 = AttrDict({'name': 'PLAYSTATION(R)3',
                ...
                'deadman':'BTN_DEAD',
                'start':'BTN_TOP',
                'ball_captured': 'BTN_JOYSTICK'}) #THESE MAPPINGS ARE FOR SONY BRAND PS3 Controller
...
class Blimpy():
    def __init__(self, g_color, lm=(27,22), rm=(9,11), bm=(25,8), am=(23,24), enb1=17, enb2=18,
                    sl =(0,0), sr=(0,0), enb3=0, #TODO input right pins for 3rd motor controller 
                    controller_map=ps3, 
        ...
        #autonomous action timers NOTE - need to control autonomous motion via time not motor strength
        ...
        #PID Settings
        self.pidx = PID(0.25,0.0,0.005, setpoint = 0.0) #(gains: P, I, D)
    ...
    def detect_goal_scored(self):
        ...
        TODO How the heck do we determine if we scored?  Maybe we don't because the game is over?'''
        goal_scored = False
        #TODO Put something here that detects the goal scoring, or manual (e.g. push a button on controller?)
        if goal_scored == True:
    ...
    def detect_object(self,obj_type):
        ...
        if self.current_mask != None:
            #NOTE- updated to use contour detection
    ...
    def detect_target_captured(self):
        if self._get_distance() < 15:
            #self.state.target_captured = True #TODO - uncomment this
            self.state.target_captured = False
        else:
            self.state.target_captured = False
    ...
    def perform_search_for_object(self,obj_type):
        ...
        if self.alt_correct == False:
            if time() - self.search_up_timer > 8.0:
                self.alt_correct = True

        elif self.alt_err >= 0: #if no april tag found or april tag is above blimp perform climb else don't mess with altitude 
            self.action.climb = 0.8
            self.alt_correct = True
        else:
    ...
    def perform_move_to_object(self,object):
        ...
        if (yaw_rate < 0 and resultx < 0) or (yaw_rate < 0 and result > 0):
            action_time = abs(resultx)-abs(yaw_rate/max_yaw_rate)
        elif (yaw_rate > 0 and resultx < 0) or (yaw_rate < 0 and result > 0):
    ...
    def observe_orient_decide(self):
        ...
        print("OODA Loop Time: ", time()- start)
